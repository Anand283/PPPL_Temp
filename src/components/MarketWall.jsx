...
const BASE_INSTRUMENTS = [
  {
    id: 'KP-A',
    name: 'Kraft Paper Grade A (BIS 1396)',
    unit: 'MT',
    base: 58500,
    volatility: 0.014,
  },
  ...
  {
    id: 'OCC',
    name: 'OCC — Old Corrugated Containers',
    unit: 'MT',
    base: 15800,
    volatility: 0.018,
  },
  {
    id: 'RSC-5',
    name: '5-Ply RSC Box — Standard (1,000+ units)',
    unit: 'unit',
    base: 148,
    volatility: 0.007,
  },
  {
    id: 'RSC-7',
    name: '7-Ply RSC Box — Heavy Duty (500+ units)',
    unit: 'unit',
    base: 218,
    volatility: 0.007,
  },
...
export default function MarketWall() {
    ...
    const id = setInterval(() => {
      ...
      setRows(prev =>
        prev.map(row => {
          ...
          const pctChange = ((newPrice - row.base) / row.base) * 100
          const dir = Math.abs(delta) < 0.001 ? 'flat' : delta > 0 ? 'up' : 'down'
          nextFlash[row.id] = dir
          return { ...row, price: newPrice, change: pctChange, dir }
        })
      )
  ...
  return (
    <section id="market" className="bg-[#0a0a0a] py-20 border-t-4 border-[#FFD700]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                <span className="text-[#FFD700] font-black text-xl md:text-2xl tracking-[0.18em] uppercase font-mono">
                  LIVE MARKET INDEX
                </span>
                {/* Pulsing live dot */}
        ...
        <div className="border border-[#1e1e1e] overflow-x-auto">
          ...
          {rows.map(row => {
            ...
            const flashDir = flashMap[row.id]
            const rowFlashBg =
              flashDir === 'up'
                ? 'bg-green-950/40'
                : flashDir === 'down'
            ...
            const signalColor = isUp ? 'text-[#00FF41]' : 'text-[#FF5555]'

            return (
              <div
                key={row.id}
                className={`grid grid-cols-12 px-4 py-4 border-b border-[#141414] font-mono items-center transition-colors duration-300 hover:bg-[#111111] min-w-[620px] ${rowFlashBg}`}
              >
                ...
                <div className={`col-span-3 text-right text-sm font-black tabular-nums transition-colors duration-300 ${priceColor}`}>
                ...
                <div
                  className={`col-span-2 text-right text-xs font-semibold tabular-nums ${
                    isUp ? 'text-[#00FF41]' : 'text-[#FF5555]'
                  }`}
                >
                  {isUp ? '+' : ''}{row.change.toFixed(2)}%
                </div>
                <div className="col-span-1 flex justify-end">
                  <ChangeIcon size={14} className={signalColor} />
                </div>
              </div>
            )
          })}
        ...
        {/* Disclaimer + CTA */}
        <div className="mt-5 border border-[#FFD700]/25 bg-[#FFD700]/[0.04] p-4 md:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-start gap-3">
            <AlertTriangle size={18} className="text-[#FFD700] flex-shrink-0 mt-0.5" />
            <div>
              <div className="text-[#FFD700] font-black text-xs uppercase tracking-[0.18em] mb-1">
                Prices Are Market-Linked &amp; Indicative Only
              </div>
              <div className="text-gray-500 text-xs font-mono leading-relaxed max-w-2xl">
                All figures are simulated indices tied to real Kraft paper &amp; OCC market
                movements. Actual contract pricing depends on volume, spec, and lead time.
          ...
          <a
            ...
            className="flex-shrink-0 flex items-center gap-2 bg-[#FFD700] text-[#111111] px-5 py-3 font-black uppercase tracking-[0.15em] text-xs hover:bg-white transition-colors duration-150 whitespace-nowrap"
          >
            <Phone size={13} />
            Call to Lock Price
          </a>
